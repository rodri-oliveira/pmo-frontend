...
export async function request(endpoint, options = {}) {
  ...
  const API_BASE_URL = isDevelopment
    ? 'http://localhost:8000/backend/v1'
    : process.env.NEXT_PUBLIC_BACKEND_URL;

    console.log('[api.js] Client-side NEXT_PUBLIC_BACKEND_URL:', process.env.NEXT_PUBLIC_BACKEND_URL);
  const url = `${API_BASE_URL}${endpoint}`;
  console.log('[api.js] Final URL:', url);
  const headers = {
...
const FILTROS_BASE_URL = API_BASE_URL;
